function [ res, mean_std_crack_block, mean_mean_crack_block, mean_std_no_crack_block, mean_mean_no_crack_block ] = block_detectV2( image, num_of_blocks, moment_order, train_mean, sigma_m, train_std, sigma_s )
...
hold on;
scatter(train_std*sigma_s, train_mean*sigma_m);
scatter(std_block(find(~block_label)), mean_block(find(~block_label)));
...
ylabel(' Block Mean Value ') % x-axis label
xlabel('Block Min Value') % y-axis label
hold on;
scatter(train_std*sigma_s, train_mean*sigma_m);
scatter(min_block(find(~block_label)), mean_block(find(~block_label)));
legend('Blocks labeled with Crack','Blocks labeled without Crack')
